...
class Enemy(pygame.sprite.Sprite):
    ...
    def __init__(self, num):
        ...
        self.num = num
        if self.num == 1:
            self.rect = self.surf.get_rect(center=(random.randint(25, SCREEN_WIDTH - 25), 0))  # SCREEN_HEIGHT)))
        elif self.num == 2:
            self.rect = self.surf.get_rect(center=(0, random.randint(25, SCREEN_HEIGHT - 25)))  # SCREEN_HEIGHT)))
...
while True:
    ...
    for entity in all_sprites:
        ...
        if entity == E1 or entity == E2:
            current_reward = entity.return_reward()
    ...
    if (P1.rect.left <= 3 or P1.rect.right >= SCREEN_WIDTH - 3 or P1.rect.top <= 3 or P1.rect.bottom >= SCREEN_HEIGHT - 3):
        reward -= 1
        current_reward = -1
        game_over = 1

    #if random.random() > 0.8:
    #    state = (torch.Tensor([P1.rect.center[0], I1.rect.center[0] + random.randint(-5, 5)]) / 400 - 0.5)
    #else:
    state = (torch.Tensor([P1.rect.center[0], P1.rect.center[1], E1.rect.center[0], E1.rect.center[1], E2.rect.center[0], E2.rect.center[1]]) / 400 - 0.5)
    ...
    if current_reward != 0 and current_reward != 0.01:
